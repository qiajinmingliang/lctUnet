...
def generate_clock(image, array, start, end):
    ...
    image_block.SetSpacing(
        (image.GetSpacing()[0] * int(1 / para.down_scale),#int
         image.GetSpacing()[1] * int(1 / para.down_scale),#int
...
class LITS_fix:
    def __init__(self, raw_dataset_path, fixed_dataset_path):
        ...
        self.raw_root_path = para.raw_coronary_data_path
        self.fixed_path = para.fix_coronary_data_path
        ...
        for ct_file in tqdm(os.listdir(self.raw_root_path + 'ct/')):
            try:
                ...
                while r < ct_array.shape[0]:
                    ...
                    ct_block_name = 'volume-' + str(idx) + '.nii.gz'
                    seg_block_name = 'segmentation-' + str(idx) + '.nii.gz'
                    sitk.WriteImage(ct_block, os.path.join(para.fix_coronary_data_path + '/ct', ct_block_name))
                    sitk.WriteImage(seg_block, os.path.join(para.fix_coronary_data_path + '/seg', seg_block_name))
                ...
                print(f"Failed to process file {ct_file}: {e}")
                continue
...
def main():
    raw_dataset_path = para.raw_coronary_data_path
    fixed_dataset_path = para.fix_coronary_data_path
