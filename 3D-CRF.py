...
for file_index, file in enumerate(os.listdir(para.test_coronary_ct_path)):
    ...
    pred = sitk.ReadImage(os.path.join(para.pred_coronary_path, file.replace('volume', 'segmentation_result')), sitk.sitkUInt8)
    ...
    #调整颜色无关特征的权重可以增强空间上邻近像素之间的相互作用，从而连接断裂部分。尝试将compat参数的值增大
    feats = create_pairwise_gaussian(sdims=(para.s1, para.s1, para.s1), shape=new_ct_array.shape)
    d.addPairwiseEnergy(feats, compat=5, kernel=dcrf.DIAG_KERNEL, normalization=dcrf.NORMALIZE_SYMMETRIC)#compat=3

    # This creates the color-dependent features and then add them to the CRF#
    #增加颜色相关特征的权重可以加强相似颜色的像素之间的相互作用，从而连接断裂部分。尝试将compat参数的值增大
    feats = create_pairwise_bilateral(sdims=(para.s2, para.s2, para.s2), schan=(para.s3,), img=new_ct_array)
    d.addPairwiseEnergy(feats, compat=10, kernel=dcrf.DIAG_KERNEL, normalization=dcrf.NORMALIZE_SYMMETRIC)#compat=10
    ...
    liver_score['recall'].append(liver_metric.get_recall())
    liver_score['precision'].append(liver_metric.get_precision())
    ...
    if not os.path.exists(para.crf_coronary_path):
        os.makedirs(para.crf_coronary_path)
    sitk.WriteImage(pred_seg, os.path.join(para.crf_coronary_path, file.replace('volume', 'crf')))

    print('dice:', liver_score['dice'][-1])
    print('recall:', liver_score['recall'][-1])
    print('precision:', liver_score['precision'][-1])
    print('--------------------------------------------------------------')
...
writer = pd.ExcelWriter('./merge/data/merge-jiaquan-a55b95t11/result-post-processing_'+para.model_name+'.xlsx')
